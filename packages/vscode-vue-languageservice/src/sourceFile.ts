...
import { useTemplateLsScriptRaw } from './virtuals/templateLsScriptRaw';
...
export function createSourceFile(
	...
	const docLsScript = useTemplateLsScriptRaw(untrack(() => vueDoc.value), computed(() => descriptor.script));
	const docLsScriptSetup = useTemplateLsScriptRaw(untrack(() => vueDoc.value), computed(() => descriptor.scriptSetup));
	...
	return {
		...
		getScriptLsDoc: untrack(() => scriptLsScript.textDocument.value),
		getScriptLsSourceMap: untrack(() => scriptLsScript.sourceMap.value),
		getTsSourceMaps: untrack(() => tsSourceMaps.value),
		getMainTsDoc: untrack(() => templateLsMainScript.textDocument.value),
		...
		getScriptSetupData: untrack(() => templateLsScript.scriptSetupAst.value),
		...
		getVirtualScriptUri: untrack(() => templateLsScript.textDocument.value?.uri),
	};
		...
		function useScriptExistValidation() {
			const result = computed(() => {
				...
				if (
					...
					&& !templateTsLs.__internal__.getTextDocumentCheck(templateLsScript.textDocument.value.uri)
				) {
