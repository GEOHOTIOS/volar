...
export function register({ sourceFiles, getTsLs }: ApiLanguageServiceContext) {
	...
	return (codeLens: CodeLens) => {
		...
		const doc = uri ? sourceFiles.get(uri)?.getTextDocument() ?? tsLs.__internal__.getTextDocumentUncheck(uri) : undefined;
		const tsDoc = tsUri ? tsLs.__internal__.getTextDocumentUncheck(tsUri) : undefined;
